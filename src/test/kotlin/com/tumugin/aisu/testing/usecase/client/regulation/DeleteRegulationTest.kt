...
class DeleteRegulationTest : BaseDatabaseTest() {
  ...
  fun testDeleteRegulation() = runTest {
    writeRegulation.deleteRegulation(user.userId, regulation.regulationId)
    Assertions.assertNull(getRegulation.getRegulation(user.userId, regulation.regulationId))
  }
  ...
  fun testDeleteRegulationWithNoPermission() = runTest {
    Assertions.assertThrows(HasNoPermissionException::class.java) {
      runBlocking {
        writeRegulation.deleteRegulation(userTwo.userId, regulation.regulationId)
      }
    }
  }
}
