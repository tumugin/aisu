...
import com.tumugin.aisu.testing.graphql.client.GetChekiMonthIdolCount
...
class GetChekiMonthIdolCountTest : BaseKtorTest() {
  ...
  @Test
  fun testGetChekiMonthIdolCount() = testAisuApplication {
    val prepareResult = prepare()
    val graphQLClient = createGraphQLKtorClient(client)
    val result = graphQLClient.execute(GetChekiMonthIdolCount()) {
      header("Cookie", prepareResult.loggedUserAndCookieValue.cookieValue)
    }

    Assertions.assertNull(result.errors)
    Assertions.assertEquals(2, result.data?.currentUserChekis?.getChekiMonthIdolCount?.size)
  }
}
